# Olo Pay Overview

Olo Pay is an E-commerce payment solution designed to help restaurants grow, protect, and support their digital ordering and delivery business. By controlling more of the payment process, Olo is able to improve the merchant and consumer experience in ways not previously possible. Olo Pay is specifically designed for restaurant digital ordering to address the challenges and concerns that we’ve heard from thousands of merchants. 

To learn more about how to get started with Olo Pay, please reach out to your Olo Customer Success representative.

# Integrating with Olo Pay

How does Olo Pay work for brands that have custom front ends or native apps?
Olo pay transactions can be processed by submitting the card data to the Olo API OR using one of the Olo Pay front end solutions, described below.

If a brand or partner is currently processing payments via the Olo API, they can begin using Olo Pay to process credit card transactions without integrating the JS library or SDK. This is an important consideration as it enables a brand or partner to move forward with rolling out Olo Pay, and implementing the JS library or SDK when development schedules allow.

Example: If a brand has a custom app, they can roll out Olo Pay without doing development to support the SDK. There will be no change in experience for the consumer on their custom app. In the future, once they integrate the SDK, they can then offer digital wallets on their mobile app and take advantage of the other Olo Pay features that come with the SDK.

<br>

#### What benefits do the Olo Pay front end solutions (JS library and SDK) offer to a 3rd party partner?

The JS Library and SDK offer the following additional features for web applications and native iOS apps:

1. Digital Wallets: Apple Pay and Google Pay
2. Enhanced fraud signals: Improved ability to identify risky transactions. (Note: fraud prevention is supported without the JS library and SDK, but using these improves and expands the data that is used to evaluate a transaction.)
3. PCI Scope Reduction: Customizable input forms to collect credit card information in a PCI-compliant format.
4. Improved integration points for better customizability.
5. Future payment features that are implemented for Olo Pay.

<br>

#### Which Olo Pay solution should I use for my application?

|                            | Olo Pay JS | Olo Pay iOS/Android SDKs |
| -------------------------- | :--------: | :----------------------: |
| **Web**                    |            |
| Credit Card Input Controls |     X      |
| Apple Pay                  |     X      |
| Google Pay                 |     X      |
| **Mobile Web**             |            |
| Credit Card Input Controls |     X      |
| Apple Pay                  |     X      |
| Google Pay                 |     X      |
| **Hybrid App**             |            |
| Credit Card Input Controls |    X\*     |          X\*\*           |
| Apple Pay                  |    X\*     |            X             |
| Google Pay                 |            |            X             |
| **Native App**             |            |
| Credit Card Input Controls |    X\*     |            X             |
| Apple Pay                  |    X\*     |            X             |
| Google Pay                 |            |            X             |

**\* Olo Pay JS Credit Card Elements and Apple Pay can technically work when wrapped in web views in mobile or hybrid technologies, although it is not ideal compared to using our native SDKs.**

**\*\* Native SDK components need to be exposed to the hybrid app framework. The process for exposing the controls varies based on the chosen hybrid app framework (e.g. Native Components for React Native, Plugins for Capacitor, etc). We have tested with React Native and Capacitor; your mileage may vary with other hybrid app technologies.**

# Web Applications

## Olo Pay JS

Olo Pay JS is a Stripe JS-powered library that allows API partners to easily add PCI-compliant credit card and digital wallet (Apple Pay and Google Pay) web elements to their checkout forms and seamlessly integrate with the Olo Ordering API. The library offers the benefits of handling validation, styling, accessibility, and responsiveness for:

- Credit card input elements: PCI-compliant credit card input elements are wrapped in Stripe-hosted iframe elements so that credit card data is secure.
- Plug-and-play mobile wallets support: Payment button that integrates with Google Chrome and Safari payment sheets to complete the checkout process.

## Using the Library

Olo Pay JS is available as a public package (@olo/pay) on the NPM Registry as well as a JavaScript file that can be included through a script tag from the Olo CDN.
Installing from the NPM Registry will include the types for Olo Pay JS for those using TypeScript, while loading the library from Olo’s CDN will accommodate more import mechanisms.

## Integration

Our library supports both ES Modules and UMD bundles for use:

### ES Modules

```sh
npm install @olo/pay
```

```ts
import { CreditCardElements, DigitalWallets } from "@olo/pay";
```

### UMD

```html
<script src="https://static.olocdn.net/web-client/olo-pay-js/olo-pay.js"></script>
```

```js
const { CreditCardElements, DigitalWallets } = window["@olo/pay"];
```

## Detailed Documentation

For a more detailed overview on using Olo Pay JS with your web application, visit the [NPM package](https://www.npmjs.com/package/@olo/pay).

## Setting up Apple Pay for Web

In order to use Apple Pay, Apple has to verify that your domain is authorized by checking a verification file hosted on your domain. Follow the steps below to complete the Apple Pay registration process:

1. Create a new file called apple-developer-merchantid-domain-association
2. Copy the text content below into the newly-created file and save it

```

```

3. Host this file in a .well-known directory at the root of your domain (i.e. https://example.com/.well-known/apple-developer-merchantid-domain-association)
4. Notify help@olo.com once this is done so we can register your domain with Apple Pay on our end


# Native iOS

## Native iOS SDK (Swift)

Olo Pay’s Native iOS SDK is an iOS framework written in Swift that allows partners to easily add PCI-compliant credit card and digital wallet (Apple Pay) functionality to their checkout flow and seamlessly integrate with the Olo Ordering API. The library offers the benefits of handling validation, styling, accessibility, and responsiveness for:

- PCI-compliant credit card input views: Ensures credit card data is secure
- Apple Pay support: Convenience methods that simplify Apple’s framework for processing payments via Apple Pay.

## Using the iOS SDK

Olo Pay’s Native iOS SDK framework will be available through the Olo Developer Portal. Once downloaded, it can be added to your Xcode project as a dependency. Olo Pay’s framework has a dependency on Stripe’s Framework, which can be added as a dependency via Swift Package Manager, Carthage, or CocoaPods.

Full documentation is included in the downloaded zip file, but we recommend starting with these three files:

```
OloPayAPIGateway.swift
OPPaymentCardDetailsView.swift
OPApplePayContextImpl.swift
```

See the full SDK documentation for details.


## Setting up Apple Pay for Native iOS

In order to use Apple Pay, you will need to register a merchant id and upload a certificate provided by Olo. Follow the steps below to complete the setup process:

1. Notify help@olo.com that you would like to set up Apple Pay for a native iOS app. Olo will provide the certSigningRequest file (Payment Processing Certificate) that you will upload to your Apple developer account.
2. Once you receive the certSigningRequest file from Olo, you will need to register a merchant ID and create a certificate with the certSigningRequest file.
    1. Register a merchant id in your Apple developer account [here](https://developer.apple.com/account/resources/identifiers/add/merchant). The recommended id format is: merchant.com.appname
    2. Go to [this](https://developer.apple.com/account/resources/identifiers/list/merchant) page in the Apple Developer Account
    3. Select the merchant id and click 'Create Certificate' in the Apple Pay Payment Processing Certificate section.
    4. When prompted, upload the certSigningRequest file. Verify cert details are correct. Click Activate.
    5. Download the certificate and send to help@olo.com. 
3. When Olo notifies you that the certificate has been added to our system, you can proceed with integrating ApplePay into your app with Xcode:
    1. In Xcode, open project settings, click the Signing & Capabilities tab, and add the Apple Pay capability.
    2. If prompted, log in to your Apple Developer account.
    3. Select the merchant ID that you created.
    4. The app is ready to accept Apple Pay.

# Native Android

## Native Android SDK (Kotlin)

Olo Pay's Native Android SDK is written in Kotlin as a compiled AAR library. It allows partners to easily add PCI-compliant credit card and digital wallet (Google Pay) functionality to their checkout flow and seamlessly integrate with the Olo Ordering API. The library offers the following benefits:

- PCI-compliant credit card views
    - Ensures credit card data is secure
    - Handles credit card validation
    - Styling options for consistent branding
    - Accessibility
- Google Pay
    - Convenience methods that simplify Google's framework for processing payments via Google Pay

## Using the Android SDK

Olo Pay's Native Android SDK library is currently available on a per-request basis. Once received, the .aar library can be added to your Android project as a dependency. Full SDK documentation will be provided with the library, but we recommend starting with these files:

```
OloPayApiInitializer.kt
OloPayAPI.kt
PaymentCardDetailsSingleLineView.kt
PaymentCardDetailsMultiLineView.kt
PaymentCardDetailsForm.kt
GooglePayContext.kt
```

See the full SDK documentation for more detailed documentation. 

**NOTE:** Maven support will be coming in a future release.

## Setting up Google Pay for Native Android

1. Enable Google Pay in your app manifest in the application tag
    ```
    <application>
        ...
        <meta-data
            android:name="com.google.android.gms.wallet.api.enabled"
            android:value="true" />
    </application>
    ```
2. Implement Google Pay: Refer to `GooglePayContext` SDK documentation for implementation details
3. Build and test your app with the Google Pay `Test` environment
4. Reach out to Google Support and send the APK built with the `Test` environment when requested
    - Google assesses the app and provides feedback if needed
    - Google provides instructions on how to agree to their Terms of Service and ensure production access is granted
5. Send your final production APK using the Google Pay `Production` environment to Google for a real-world test which includes a few transactions
6. If all tests pass, Google clears you to publish the APK
7. Notify Google when your APK is 100% visible to users


# Submitting a Basket via the Ordering API

The iOS, Android, and JS SDKs can be used to generate a payment method object that includes a payment method ID. The payment method ID is passed with the order details in the [Basket Submit](/docs/load/ordering-api#operation/SubmitOrderwithSinglePayment) endpoint in the `token` field. This replaces the current need to pass credit card PANs and CVVs to Olo.



# Changelog

**11/09/2021**

**Documentation Update:** Added the Olo Pay feature matrix to the [Integrating with Olo Pay section](#section/Integrating-with-Olo-Pay) so you can see which solution is right for your application.

**10/01/2021**

#### Android SDK

The Olo Pay SDK for Android is now available, allowing you to quickly and easily process Olo Pay payments in your native Kotlin or Java app. To learn more see [this](#section/Native-Android) Olo Pay documentation

**07/29/2021**

* Added new intro per manager feedback
* Adjusted section headers for clarity based on new intro

**07/14/2021**

* Expanded **Olo Pay - Native iOS SDK** section
* Rearranged several sections for clarity

**07/02/2021**

* Added **Setting up Apple Pay for Web** section

# Contact Us

**Official Developer Support Hours:**

**M-F, 9am - 8:00pm ET (6am - 5:00pm PT)**

If you have any technical questions not adequately addressed by this documentation, please feel free to reach out to Olo's Developer Support team at developersupport@olo.com.

We will do our best to address your question(s) as soon as possible! Please allow for a 24-hour response time within the business week (note: not an SLA) for most incoming technical questions. Tickets better suited for other Olo support teams will be reassigned.
